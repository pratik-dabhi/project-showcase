...
module.exports = { candidate : (req, res) => {
    db.query(`SELECT * FROM option_master where s_id='1'`,function (error, results, fields) {
        if (error) throw error; 
        gender=(results);
    });
    db.query(`SELECT * FROM option_master where s_id='3'`,function (error, results, fields) {
        if (error) throw error; 
        course=(results);
    });
    db.query(`SELECT * FROM option_master where s_id='4'`,function (error, results, fields) {
        if (error) throw error; 
        lang=(results);
    });
    db.query(`SELECT * FROM option_master where s_id='5'`,function (error, results, fields) {
        if (error) throw error; 
        tech=(results);
    });
    db.query(`SELECT * FROM option_master where s_id='6'`,function (error, results, fields) {
        if (error) throw error; 
        loc=(results);
    });
    db.query(`SELECT * FROM option_master where s_id='7'`,function (error, results, fields) {
        if (error) throw error; 
        rel=(results);
    });
    db.query(`SELECT * FROM state_master`,function (error, results, fields) {
        if (error) throw error; 
        state=(results);

        jwt.verify(req.cookies.user,'pratik',async (e,data)=>{
            if(data && data.code == 1)
            res.render("index",{gender:gender,state:state,course:course,lang:lang,tech:tech,loc:loc,rel:rel});
            else 
...
, city_api: (req,res)=>{
    ...
    db.query(`select * from state_master where state_value='${val}'`,(error1,data)=>{
        db.query(`select city_value from city_master where state_id='${data[0].state_id}'`,(error,res1)=>{
        if(error) throw(error);
        res.json({res1});
            });
        });
...
,candidate_data: (req,res)=>{
    ...
    db.query(`select * from basic_details where candidate_firstname like'${fname}'or candidate_lastname like '${lname}' or candidate_surname like '${sname}' or candidate_designation like '${designation}' or candidate_city like '${city}' `,function(error,result,fields){
        if(error) throw(error);
        data = (result);
        res.render('list',{data:data,search:search});
    })
},

//app.post
candidate_details : async(req,res)=>{
   ...
   console.log('161--'+language,read_english,read_gujarati,read_hindi);
    ...
    if(typeof(language)=='string'){
        ...
        if(`${language[i]}` == 'gujarati'){
            ...
            let aq1 = await queryExecuter(sq1);
            ...
            let aq2 =await queryExecuter(sq2);
            ...
            let data = await queryExecuter(sq3);
...
//--------DISPLAY LIST OF RECORDS ---------
 ...
 show : async(req,res)=>{
    ...
    let a1 = await queryExecuter(q1);
    data = a1;
    jwt.verify(req.cookies.user,'pratik',async (e,verify)=>{
        if(verify && verify.code == 1)
        res.render('list',{data:data,search:search,verify:verify});
        else 
            res.render('login',{err_msg:''})
    })    
    
 },
//-------DELETE API FOR DELETE SINGLE RECORD ON CLICK ON DELETE --------
//app.get
...
//--------DELETE ALL API FOR DELETE SELECTED RECORDS ------------
//app.post
delete_all_api : (req,res)=>{
    let c_ids = req.body.arrid;
    c_ids.forEach(e =>{
        db.query(`update basic_details set isDelete='Yes' where candidate_id=${e}`,(error,result,fiels)=>{
            if(error) throw (error);
            console.log(`Row ${e} data is deleted!!`);    
        })
    })
    db.query(`select candidate_id,candidate_firstname , candidate_lastname , candidate_surname , candidate_designation , candidate_city,isdelete from basic_details where isdelete='no'`,function(error,result,fields){
        if(error) throw(error);     
           res.json({result});
    })
...
update_api :async (req,res)=>{
   ...
   var startdate = req.body.startdate;
   var enddate = req.body.enddate;
   console.log('462---'+startdate,enddate);
    ...
    // use to update basic details
     ...
     let SQ1 = await queryExecuter(s1);
     console.log("Basic details updated!!!");
        ...
        let ae1 =await queryExecuter(e1);
    ...
    //Experience -------------------------------
        ...
        let aex1 =await queryExecuter(ex1);
        let exid1=[];
...
// language -----------------
    ...
    if(typeof(language) == 'string'){
        ...
        let aq1 = await queryExecuter(query1);
        ...
        let aq2 = await queryExecuter(query2);
        if(aq2 != ''){
        ...
        let aq3 = await queryExecuter(query3);
        if(aq3 != ''){
        ...
        let aq4 = await queryExecuter(query4)
        if(aq4 != ''){
        ...
        let aq5 = await queryExecuter(query5)
        if(aq5 != ''){
        ...
        let uid1 =await queryExecuter(uid);
